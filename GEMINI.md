...
### 额外修复：chinese 字段要写完整中文翻译
答对后反馈面板会显示 `sentence + " = " + chinese`，例如 "This is a lion. = 狮子"。
但学生需要看到**完整中文句子**，不是单词。

修复要求：
- 所有题目的 `chinese` 字段必须是**完整中文句子翻译**，不能只是单词
- 例如：`"chinese": "狮子"` → `"chinese": "这是一只狮子"`
- `"chinese": "夹克"` → `"chinese": "我穿了一件夹克"`
- 中文翻译要和 `sentence` 字段对应

### 验证
修完后统计：改了多少道题，确认所有选择题都是4个选项，所有 chinese 字段都是完整句子。

---

## 任务48：替换带英文标注的动物图片（P0）

### 问题
当前 `assets/images/` 中的动物图片（lion.png, monkey.png 等）上面印着英文单词（如 "LION"、"MONKEY"），导致看图选词的题目直接漏答案——学生看图就知道选什么，完全没有练习效果。

### 修复要求
1. 用新图片替换 `assets/images/` 中所有**带英文文字标注**的动物图片
2. 新图片要求：
   - 只有卡通动物图案，**不能有任何英文文字**
   - 风格统一（卡通、彩色、适合小学生）
   - 尺寸 200x200 左右，文件 <100KB
3. 需要替换的图片（至少检查）：lion.png, monkey.png, bear.png, cat.png, dog.png, bird.png, horse.png, rabbit.png, panda.png, tiger.png, elephant.png
4. 同时检查 Unit 2-4 的图片（衣服、食物、时钟等）是否也有类似问题

### 图片来源
- 可以用 AI 生成（DALL-E / Stable Diffusion）
- 或者从免费图库下载（Flaticon, Freepik 等，注意版权）
- 必须是 PNG 透明背景

---

## 任务49：写作模块题目缺失提示文字（P0）

### 问题
写作模块（U1L2 写作·练习）只显示4个字母（n, a, x, i），没有任何题目提示。学生完全不知道要做什么。

### 修复要求
1. 检查 `modules/writing.js` 的渲染逻辑，确认所有题型都有题目/提示显示
2. 写作题必须显示：
   - 题目提示（如：拼出这个动物的名字）
   - 图片或中文提示（让学生知道要拼什么词）
   - 例如：显示猴子图片 + "请拼出这个动物的英文名" → 学生排列字母
3. 检查所有16课的 writing 题库，确认每道题都有 `question`/`word`/`image` 等必要字段
4. 如果字段缺失，在题库中补上

---

## 任务50：口语模块录完音下一题按钮卡死+UI优化（P0）

### 问题
1. **录完音后"下一题"按钮灰色不可点** — 学生卡住无法继续
2. **录音状态不清晰** — 不知道是否在录音、录音是否结束
3. **"Listen to demo first"是英文** — 三年级学生看不懂

### 修复要求
1. 录音完成后（无论评分多少）必须启用"下一题"按钮
2. 录音状态要有明确视觉反馈：
   - 未录音：🎙️ 点击开始录音
   - 录音中：🔴 录音中...（红色闪烁动画）
   - 录音完成：✅ 录音完成！
3. 星星评分要准确反映分数，不能91分就满星：
   - 100分 = ⭐⭐⭐⭐⭐
   - 80-99分 = ⭐⭐⭐⭐（4颗）
   - 60-79分 = ⭐⭐⭐（3颗）
   - 40-59分 = ⭐⭐（2颗）
...
## 任务51：反馈面板布局优化+文案优化（P1）

### 问题
1. **反馈面板遮挡选项** — 答对后底部弹出的面板把下面的选项卡片挡住了
2. **文案过于夸张** — "非常好！"看起来奇怪

### 修复要求

#### A. 布局修复
答对/答错后的反馈面板不要遮挡题目内容，两种方案选一：
- 方案1（推荐）：反馈面板弹出时，把题目区域往上滚，确保反馈面板和选中的答案都能看到
- 方案2：反馈面板改为覆盖整个屏幕的半透明遮罩，居中显示反馈内容

#### B. 文案优化
答对后的随机鼓励语要自然、适合三年级小学生：
- 删除："非常好！"、"太厉害了！"等过于夸张的表达
- 改为简洁自然的鼓励：
  - "✓ 答对了！"
  - "✓ 正确！"
  - "✓ 很棒！"
